        ...
        return (
            <div className={deleted ? styles.cardDeleted : styles.card}>
                <div className={styles.info}><h4 ><Link to={`/users/${author}`}>{author}</Link>@{created_at && created_at.slice(0, 10)} </h4>
                    {currentUser === author && <button className={!deleted ? styles.deleted : styles.delete} onClick={this.removeComment} disabled={deleted}>{deleted ? "Comment Deleted" : "Delete"}</button>}</div>
